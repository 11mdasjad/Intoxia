            ...
            <div class="max-w-4xl">
                ...
                <div class="flex flex-wrap gap-2 mb-4">
                    @if($caseStudy->industry)
                        <span class="px-3.5 py-1.5 rounded-full bg-blue-50 border border-blue-200 text-blue-700 text-xs font-mono font-bold uppercase shadow-sm">
                            {{ $caseStudy->industry }}
                        </span>
                    @endif
            ...
            <div class="lg:col-span-8 space-y-14">
                ...
                @if($caseStudy->challenge)
                    <div id="challenge" class="scroll-mt-28 space-y-4">
                        <div class="text-xs font-mono text-rose-700 font-bold uppercase tracking-widest">01 — The Challenge</div>
                ...
                @if($caseStudy->solution)
                    <div id="solution" class="scroll-mt-28 space-y-4 border-t border-slate-200 pt-12">
                        <div class="text-xs font-mono text-blue-700 font-bold uppercase tracking-widest">02 — The Solution</div>
                ...
                @if($caseStudy->architecture || $caseStudy->design_approach)
                    <div id="architecture" class="scroll-mt-28 space-y-6 border-t border-slate-200 pt-12">
                        <div class="text-xs font-mono text-blue-700 font-bold uppercase tracking-widest">03 — Architecture & Stack</div>
                        <h2 class="text-2xl sm:text-3xl font-bold text-slate-900">System Architecture & Design</h2>
                        
                        @if($caseStudy->design_approach)
                            <div class="p-6 rounded-2xl bg-white border border-slate-200 text-slate-700 text-sm leading-relaxed shadow-sm">
                                <strong class="text-slate-900 block mb-2 font-mono text-xs uppercase tracking-wider text-blue-700">Design & UX Philosophy</strong>
                            ...
                            <div class="pt-4">
                                ...
                                <div class="flex flex-wrap gap-2">
                                    @foreach($caseStudy->technologies_used as $tech)
                                        <span class="px-3.5 py-1.5 rounded-lg bg-blue-50 border border-blue-200 text-blue-700 text-xs font-mono font-semibold">
                                            {{ $tech }}
                                        </span>
                                    @endforeach
                                </div>
                ...
                @if($caseStudy->results && is_array($caseStudy->results))
                    <div id="results" class="scroll-mt-28 space-y-6 border-t border-slate-200 pt-12">
                        <div class="text-xs font-mono text-emerald-600 font-bold uppercase tracking-widest">04 — Commercial Impact</div>
                        <h2 class="text-2xl sm:text-3xl font-bold text-slate-900">Results Achieved</h2>
                        
                        <div class="grid grid-cols-1 gap-4">
                            @foreach($caseStudy->results as $result)
                                <div class="p-5 rounded-2xl bg-emerald-50/80 border border-emerald-200 flex items-center space-x-3 text-slate-700 text-sm">
                ...
                @if($caseStudy->testimonial_quote)
                    <div class="p-8 rounded-3xl bg-blue-50/60 border border-blue-200 relative">
                        <div class="text-4xl text-blue-500 font-serif leading-none mb-3">“</div>
                        <p class="text-base text-slate-700 italic leading-relaxed">
                            {{ $caseStudy->testimonial_quote }}
                        </p>
                        <div class="mt-4 pt-4 border-t border-blue-200/60 text-xs font-mono text-slate-600">
            ...
            <div class="lg:col-span-4 space-y-8">
                <div class="glass-panel rounded-3xl p-6 sm:p-8 border border-blue-500/20 sticky top-28 space-y-6">
                    ...
                    <nav class="space-y-2 text-sm">
                        <a href="#challenge" class="block py-1.5 text-slate-600 hover:text-blue-600 font-mono transition-colors">01. The Challenge</a>
                        <a href="#solution" class="block py-1.5 text-slate-600 hover:text-blue-600 font-mono transition-colors">02. The Solution</a>
                        <a href="#architecture" class="block py-1.5 text-slate-600 hover:text-blue-600 font-mono transition-colors">03. Architecture & Tech</a>
                        <a href="#results" class="block py-1.5 text-slate-600 hover:text-blue-600 font-mono transition-colors">04. Results & Metrics</a>
                    </nav>
