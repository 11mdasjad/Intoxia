...
<article class="glass-card rounded-2xl overflow-hidden group flex flex-col justify-between">
    ...
    <div class="relative h-48 bg-slate-900 overflow-hidden">
        ...
        @else
            <div class="w-full h-full bg-gradient-to-br from-blue-950/40 via-slate-900 to-indigo-950/40 flex items-center justify-center p-6 text-center border-b border-white/10">
                <span class="font-mono text-xs text-blue-200 font-bold uppercase tracking-widest">{{ $post->category->name ?? 'Technology' }}</span>
            </div>
        @endif

        @if($post->category)
            <div class="absolute top-3 left-3">
                <span class="px-2.5 py-1 rounded-md text-[11px] font-mono font-bold bg-white/90 backdrop-blur-md text-blue-700 border border-slate-200 shadow-sm">
                    {{ $post->category->name }}
                </span>
            </div>
        @endif
        ...
        @if($post->reading_time)
            <div class="absolute bottom-3 right-3">
                <span class="px-2 py-0.5 rounded text-[10px] font-mono text-white bg-black/70 backdrop-blur-sm">
    ...
    <div class="p-6 flex-grow flex flex-col justify-between">
        <div>
            ...
            <h3 class="text-lg font-bold text-slate-900 group-hover:text-blue-600 transition-colors tracking-tight leading-snug">
        ...
        <div class="mt-6 pt-4 border-t border-slate-200 flex items-center justify-between">
            ...
            <a href="{{ route('insights.show', $post->slug) }}" class="text-xs font-mono font-semibold text-blue-600 group-hover:text-blue-700 inline-flex items-center space-x-1 transition-colors">
